  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#2A2D30] pb-4">
        ...
        <div className="flex flex-wrap items-center gap-1.5 bg-[#151719] p-1.5 rounded-xl border border-[#2A2D30]">
          <button
            onClick={() => setActiveTab("ALL")}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${
              activeTab === "ALL"
                ? "bg-[#1C1E20] text-[#F9732F] border border-[#F9732F]/20"
                : "text-[#9CA3AF] hover:text-[#F5F2ED]"
            }`}
          >
      ...
      {filteredEvidences.length === 0 ? (
        <div className="card-surface rounded-2xl p-8 text-center text-[#9CA3AF] text-sm font-sans">
