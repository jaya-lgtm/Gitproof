...
export function RankedProjectCard({ match, rank }: RankedProjectCardProps) {
  ...
  return (
    <div className="bg-slate-900/90 border border-slate-800 hover:border-slate-700/80 rounded-2xl p-6 transition-all duration-200 shadow-xl space-y-4">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <a
              ...
              className="text-lg font-bold text-slate-100 hover:text-emerald-400 transition-colors inline-flex items-center gap-1.5"
            >
              <span>{repo.name}</span>
        ...
        <div className={`px-3 py-1.5 rounded-xl border text-sm font-extrabold font-mono flex items-center gap-1.5 ${scoreColor}`}>
          ...
          <span className="text-[10px] font-normal uppercase tracking-wider">Match</span>
        </div>
      </div>

      {/* Match Reason */}
      <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-800/80 text-xs text-slate-300 leading-relaxed">
        <span className="font-semibold text-emerald-400">Match Analysis: </span>
        {match.matchReason}
      </div>
            ...
            <div className="mt-2.5 p-3.5 bg-slate-950/90 rounded-xl border border-slate-800 space-y-2 text-xs animate-fade-in">
              ...
              <div className="space-y-2">
                {match.evidenceQuotes.map((quote, idx) => (
                  <div key={idx} className="bg-slate-900 p-2.5 rounded border border-slate-800 font-mono space-y-1">
